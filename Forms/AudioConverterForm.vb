...
#Region " Designer "
    ...
    Protected Overloads Overrides Sub Dispose(disposing As Boolean)
        ...
        If disposing Then
            ...
            End If
        End If
        'If AudioConverterForm.DisplayNameCache IsNot Nothing Then
...
#End Region
    ...
    'Public Shared DisplayNameCache As Dictionary(Of Long, String) ' or use dictionary as thread safe
    ...
    Private ReadOnly StatUpdateTaskA As Action =
        Sub()
            Do While PopulateIter < 900 AndAlso AudioConverterMode
                Dim t As String = PopulateIter.ToInvString & PopulateTask?.Status.ToString & PopulateWSW.ElapsedTicks / 10000 & "msW" & PopulateTaskS.ToInvString & "PopTS" & PopulateRSW.ElapsedTicks / 10000 & "msR|WaitPT"
                t &= PopulateTaskW?.Status.ToString & IndexSWatch.ElapsedTicks / 10000 & "msIx|SwT" & SWt.ElapsedTicks / 10000 ' & "VC" & CellVNeededCount.ToInvString
                t &= "MC" & MediaInfo.Cache.Count '& "DC" & DisplayNameCache.Count 
    ...
    Public Sub New()
        ...
        AudioCL = New CircularList(Of AudioProfile)(64)
        ...
        'DisplayNameCache = New Dictionary(Of Long, String)(64)
        ...
        'indexColumn.ValueType = GetType(integer)
    ...
    Private Sub UpdateCMS(sender As Object, e As CancelEventArgs) Handles CMS.Opening
        ...
        'Dim iterations = 100000000
        'Dim t1 As String = " u sdgsdfgh34t6456$%&sdfg er 834 t934 dfbdx e r i r9bdrfpbfg df   df9e r9034  0 45iod fd fo^CFHdfhxdfghfcgjFGHDFJibjk897uhgjhjkunjioji87y7y8jijiJKLjkJIIiJ8w469*(8*(&@#$%^&efgmdklfgriodrfhdriotklrthkoKJNfgskod o"
        'Dim t2 As String = "1d345rdg RR Hdrth rdt erg ei oiser sergsrtigsergsdc2345dfg edx gds ffg df dfg f drt fdg456cgnfgn  eagjsergiose  rooer gperoperi er grrgoje fgopxeiroper g0450w45ty90w dfbgdui sdif giY uy ui7^  e rfgo3 498seufg uidfuier hdfui gdfuedfui  dfuid f uidfd f uiodf dfjhijkdxf"
        'Dim r1 As Integer
        'Dim r2 As Integer
        ...
        '    '    If i > 10 Then
        '    '        t1 = PopulateIter.ToInvString & PopulateTask?.Status.ToString & PopulateWSW.ElapsedTicks / 10000 & "msW " & PopulateTaskS.ToInvString & "PopTS " & PopulateRSW.ElapsedTicks / 10000 & "msR WaitPT"
        '    '        t1 &= PopulateTaskW?.Status.ToString & IndexSWatch.ElapsedTicks / 10000 & "msIx SwT" & SWt.ElapsedTicks / 10000 ' & "VC" & CellVNeededCount.ToInvString
        '    '        t1 &= "DC " & DisplayNameCache.Count & "MC" & MediaInfo.Cache.Count
        '    '    End If
        'Next i
        'SWt.Stop()
        ...
        '    'If PopulateIter > -10 Then r2 = EnumExtensions.ToString(DirectCast(AudioCL(0), GUIAudioProfile).Params.Codec)
        '    'Integer.TryParse(t1, r2)
        '    '    StatTextSB.Clear()
        '    '    StatTextSB.Append("Pos: ").Append(n + 2).Append("  |  Sel: ").Append(n - 500000).Append(" / Tot: ").Append(n * 3)
        '    '    t2 = StatTextSB.ToString()
        '    'End If
        'Next n
        'SWt.Stop()
        ...
        'AddHandlersDGV()
        'Task.Delay(15).Wait()
        'MsgInfo($"{iterations:N0}it.StringLen {tt1}ms", $"Gethash: {SWt.ElapsedTicks / 10000}ms| {t1} {t1} | {r1}|{r2}")
        '''End If
        ...
        CMS.Add("Remove duplicates", Sub()
                                         ...
                                         ElseIf MsgQuestion(AudioCL.Count - uAR.Length & " duplicated file(s) found." & BR & "Remove from Grid View ?") = DialogResult.OK Then
                                             ...
                                             If ocF IsNot Nothing Then  ' AndAlso uAR.Contains(ocF)
                                                 dgvAudio.CurrentCell = dgvAudio.Rows.Item(Array.FindIndex(uAR, Function(itm) String.Equals(itm.File, ocF))).Cells.Item(occ)
                                             End If
                                             AutoSizeColumns()
                                         End If
    ...
    Private Sub UpdateControls(Optional SelRowsCount As Integer = -1, Optional CurrentRowI As Integer = -1)
        ...
        StatTextSB.Clear()
        StatTextSB.Append("Pos: ").Append((CurrentRowI + 1).ToInvString).Append("  |  Sel: ").Append(SelRowsCount.ToInvString).Append(" / Tot: ").Append(rC.ToInvString)
        Me.BeginInvoke(Sub()
                           'IndexSWatch.Restart()
                           If rC > 0 Then
    ...
    Public Shared FilesDictindex As Integer
    Private Sub ProcessInputAudioFiles(Files As String())
        Dim fC As Integer = Files.Length

        'Dim ap1 As AudioProfile
        'ap1 = p.Audio0
        'Dim ap2 = ObjectHelp.GetCopy(ap1)
        'ap2.File = Files(0)
        'ap2.FileKeyHashValue = 1234
        'ap2.SourceSamplingRateValue = 4321
        'DirectCast(ap2, GUIAudioProfile).DisplayNameValue = "qaz"
        'ap2.DisplayNameValue = "zxc"
        'Dim ap3 = ObjectHelp.GetCopy(ap2)


        Dim filesHash(fC - 1) As Long
        Dim fSortHashT = Task.Run(Sub()
                                      SWt.Restart()
                                      Array.Sort(Files)
                                      Parallel.For(0, fC, Sub(i) filesHash(i) = Files(i).GetHashCode + File.GetLastWriteTime(Files(i)).Ticks)
                                      SWt.Stop()
                                  End Sub)
        ...
        SWt.Stop()
        Dim sw1 = SWt.ElapsedTicks / 10000
        SWt.Restart()
        ...
        If Not fSortHashT.IsCompleted Then fSortHashT.Wait()

        Dim uc As Integer
        uc = filesHash.Union(filesHash).Count()
        If uc <> filesHash.Length Then MsgInfo("Hash Collision!", uc & " | " & filesHash.Count)

        SWt.Stop()
        Dim sw2 = SWt.ElapsedTicks / 10000
        SWt.Restart()
        ...
        Parallel.For(0, fC, 'New ParallelOptions With {.MaxDegreeOfParallelism = 1},
                         Sub(i As Integer)
                             Dim apN = ObjectHelp.GetCopy(ap)
                             apN.File = Files(i)

                             apN.FileKeyHashValue = filesHash(i)

                             Dim fExt As String = apN.File.Ext
                             If FileTypes.VideoAudio.Contains(fExt, StringComparer.Ordinal) Then
                                 apN.Streams = MediaInfo.GetAudioStreams(apN.File, filesHash(i))
                                 ...
                                 Else
                                     ...
                                     Log.WriteLine("-->Audio stream not found, skipping: " & BR & apN.File)
                                     Return
                                 End If
                                 ...
                                 If Not autoStream AndAlso apN.Stream IsNot Nothing AndAlso apN.Streams.Count > 1 Then
                                     SyncLock dummyStream
                                         If Not autoStream Then
                                             ...
                                             .Text = "Please select an audio stream for: " & BR & apN.File.ShortBegEnd(60, 60)}
                             ...
                             'clth.Add(ObjectHelp.GetCopy(apN))
                         End Sub)

        SWt.Stop()
        Dim sw3 = SWt.ElapsedTicks / 10000
        SWt.Restart()
        ...
        SWt.Stop()
        laThreads.Text = $"{sw1:F3}|{sw2:F3}|{sw3:F3}|{SWt.ElapsedTicks / 10000:F3}"
                                                           ...
                                                           Dim pl = Parallel.For(0, arP.Length, New ParallelOptions With {.MaxDegreeOfParallelism = CInt(CPUs / 2)},
                                                                                 Sub(i, pls)
                                                                                     If PopulateTaskS <= 0 OrElse Not AudioConverterMode Then
                                                                                         ...
                                                                                         Return
                                                                                     Else
                                                                                         'MediaInfo.SetMediaInfoCache(arP(i).File, arP(i).FileKeyHashValue)
                                                                                         'arP(i).SetDisplayNameCache()
                                                                                         If arP(i).DisplayName IsNot Nothing Then
                                                                                         End If
                                                                                     End If
                                                                                 End Sub)
    ...
    Private Sub bnAudioAdd_Click(sender As Object, e As EventArgs) Handles bnAudioAdd.Click
                ...
                Case "folder"
                    Using dialog As New FolderBrowserDialog With {.RootFolder = Environment.SpecialFolder.MyComputer}
                        If dialog.ShowDialog = DialogResult.OK Then
                            ...
                            avf = Directory.GetFileSystemEntries(dialog.SelectedPath, "*.*", opt).Where(Function(file)
        ...
        If avf?.Length > 0 Then ProcessInputAudioFiles(avf) Else MsgWarn("No Audio or Video files found here.")
    End Sub
    ...
    Private Sub bnAudioEdit_Click(sender As Object, e As EventArgs) Handles bnAudioEdit.Click
        ...
        If ap.EditAudioConv() = DialogResult.OK Then
            ...
            If srC > 1 Then
                AudioSBL.RaiseListChangedEvents = False
                Dim isGap = TypeOf ap Is GUIAudioProfile
                ...
                Parallel.For(0, rC, Sub(r)                      ' 
                                        If dgvAudio.Rows.GetRowState(r) >= &B1100000 AndAlso r <> sr0idx Then
                                            ...
                                            ap0.Stream = apn.Stream ' Forced,Default if stream can be problem???
    ...
    Private Sub DeleteRows()
        ...
        Else
            ...
            'If srC > rC / 2 Then DisplayNameCache.Clear()
    ...
    Private Sub laAC_DoubleClick(sender As Object, e As EventArgs) Handles laAC.DoubleClick
        ...
        SyncLock SLock
            ...
            Dim ocF As String = If(rC > 0 AndAlso occ.Y >= 0, AudioSBL.Item(occ.Y).File, Nothing)
            AudioSBL.RemoveSort()
            AudioSBL.RaiseListChangedEvents = False
            Console.Beep(700, 60)
            ...
            Dim isGAP = rC > 0 AndAlso TypeOf AudioCL.Item(0) Is GUIAudioProfile
            If Not PopulateTask.IsCompleted AndAlso Not PopulateWSW.IsRunning Then MsgInfo("Populate still running !") 'PopulateTask.Wait()
            ...
            IndexSWatch.Stop()
            Dim sw1 = IndexSWatch.ElapsedTicks / 10000
            IndexSWatch.Restart()
            ...
            If rC > 0 Then Parallel.For(0, uAR.Length, Sub(i)
                                                       ...
                                                       End Sub)
            IndexSWatch.Stop()
            Dim sw2 = IndexSWatch.ElapsedTicks / 10000
            Console.Beep(900, 60)
            ...
            If ocF IsNot Nothing Then  ' AndAlso uAR.Contains(ocF)
                dgvAudio.CurrentCell = dgvAudio.Rows.Item(Array.FindIndex(uAR, Function(itm) String.Equals(itm.File, ocF))).Cells.Item(If(occ.X >= 0, occ.X, 2))
            End If
            'dgvAudio.SelectAll()
            ...
            IndexSWatch.Stop()
            laThreads.Text = ($"{sw1} {sw2} {IndexSWatch.ElapsedTicks / 10000}")
